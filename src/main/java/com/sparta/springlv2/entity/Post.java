...
public class Post extends Timestamped{ // 타임스탬프는 포스트 entity에 상속이 되어 사용
    ...
    //기본 키 생성 방법을 지정하는 어노테이션, strategy 속성을 사용하여 여러방식 중 GenerationType.AUTO를 사용하여 JPA가 알아서 기본 키를 설정하게 함
    private Long id;
    ...
    // fetch는 속성을 Eager과 Lazy로 설정 가능. Lazy로 설정할 경우 객체를 실제로 필요한 순간에 가져오게 함
